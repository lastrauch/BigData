...
netflix_shows.columns = ['show_id', 'type', 'Title', 'director', 'cast', 'country', 'date_added', 'release_year', 'rating', 'duration', 'listed_in', 'description']
...
imdb_combined = pd.concat([genre_imdb.set_index('tconst'),rating_imdb.set_index('tconst')], axis=1)
imdb_combined.drop(columns=['titleType', 'primaryTitle', 'isAdult', 'startYear', 'endYear', 'runtimeMinutes', 'numVotes'], inplace=True)
...
netflix_collection_imdb = pd.merge(netflix_collection, imdb_combined, on='Title', how='outer')
netflix_collection_imdb.to_csv('/Users/lstrauch/Documents/Uni/Semester_3/Big_Data/Projekt/all_netflix_imdb_combined.csv', encoding='utf-8', index=False)